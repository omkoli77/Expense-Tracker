...
export default function ExpenseItem({ expense, onDelete, deleting }) {
  const chipClass = CATEGORY_STYLES[expense.category] || CATEGORY_STYLES.Other;

  return (
    <li className="flex items-center gap-3 py-3 px-1 sm:px-2 group">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted mt-0.5 font-mono">{formatDate(expense.date)}</p>
      </div>
      ...
      <button
        ...
        className="ml-1 text-muted hover:text-rust disabled:opacity-40 transition-colors p-1.5 rounded-md hover:bg-rust-light"
      >
        <svg
